...
type OverviewData = {
  ...
  attention: {
    overdueInvoices: { id: string; number: string; customer: string; total: number; currency: string; dueDate: string; daysLate: number }[];
    expiringQuotes: { id: string; number: string; customer: string; total: number; currency: string; validUntil: string; daysLeft: number }[];
  ...
  activity: { type: string; label: string; description: string; amount: number; currency: string; date: string; status: string; href: string }[];
};
...
}: { label: string; value: string; hint?: string; tone?: 'default' | 'positive' | 'warning' | 'danger'; href?: string }) {
  const toneClass =
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400">Revenue vs Expenses</p>
          <p className="text-sm font-semibold text-zinc-900 mt-0.5">Last 12 months</p>
        </div>
        <div className="flex gap-3 text-xs text-zinc-500">
          <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-green-500" /> Revenue</span>
      ...
      <div className="flex items-end gap-2 h-48">
        {months.map((m) => {
          ...
          return (
            <div key={m.key} className="flex-1 flex flex-col items-center gap-1" title={`${m.label}: Rev ${fmt(m.revenue)} / Exp ${fmt(m.expenses)}`}>
              <div className="flex items-end gap-0.5 h-full w-full justify-center">
                <div className="w-2.5 rounded-t-sm bg-green-500 transition-all" style={{ height: `${rPct}%` }} />
                <div className="w-2.5 rounded-t-sm bg-red-400 transition-all" style={{ height: `${ePct}%` }} />
              </div>
...
function AttentionSection({ attention }: { attention: OverviewData['attention'] }) {
  const sections = [
    ...
    },
    {
      ...
      items: attention.expiringQuotes.map((q) => ({
        ...
        secondary: q.daysLeft === 0 ? 'Expires today' : `${q.daysLeft}d left · valid to ${fmtDate(q.validUntil)}`,
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="mb-4">
        <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400">Needs Your Attention</p>
        <p className="text-sm font-semibold text-zinc-900 mt-0.5">Action items across the business</p>
      </div>
          ...
          <div key={s.key} className={`rounded-lg border ${s.tone} p-3`}>
            ...
            {s.items.length === 0 ? (
              ...
              <ul className="space-y-1.5">
                {s.items.map((it) => (
                  <li key={it.key}>
                    <Link href={it.href} className="flex items-center justify-between gap-2 rounded px-1.5 py-1 text-xs hover:bg-white/50">
...
function QuickActions() {
  const actions = [
    ...
    { href: '/admin/quotes', label: 'Quote', icon: '📝' },
    { href: '/admin/receipts', label: 'Receipt', icon: '🧾' },
    { href: '/admin/delivery-notes', label: 'Delivery', icon: '🚚' },
    { href: '/admin/expenses', label: 'Expense', icon: '💸' },
    { href: '/admin/products', label: 'Product', icon: '📦' },
  ];
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
      <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-3">Quick Actions</p>
...
function ActivityFeed({ activity }: { activity: OverviewData['activity'] }) {
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
      <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-3">Recent Activity</p>
      {activity.length === 0 ? (
        ...
        <ul className="divide-y divide-zinc-100">
          {activity.map((a, i) => (
            <li key={`${a.type}-${i}-${a.label}`}>
              <Link href={a.href} className="flex items-center gap-3 py-2 -mx-1 px-1 rounded hover:bg-zinc-50">
                <span className={`text-lg ${TYPE_TONE[a.type] || 'text-zinc-500'}`}>{TYPE_ICON[a.type] || '•'}</span>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-zinc-800 truncate">{a.label}</p>
                  <p className="text-xs text-zinc-500 truncate">{a.description} · {fmtDate(a.date)}</p>
                </div>
                <span className="text-sm font-semibold text-zinc-900 tabular-nums">{fmt(a.amount, a.currency)}</span>
              </Link>
            </li>
          ))}
        </ul>
...
function TopCustomers({ customers }: { customers: OverviewData['topCustomers'] }) {
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
      <p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-3">Top Customers (90d)</p>
      {customers.length === 0 ? (
        <p className="text-sm text-zinc-400">No invoices in the last 90 days.</p>
      ) : (
        <ul className="space-y-2.5">
          {customers.map((c) => {
            const paidPct = c.totalInvoiced > 0 ? Math.round((c.totalPaid / c.totalInvoiced) * 100) : 0;
            return (
              <li key={c.name} className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-sm font-semibold text-zinc-900 tabular-nums">{fmt(c.totalInvoiced)}</span>
                  </div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <div className="flex-1 h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                      <div className="h-full bg-green-500" style={{ width: `${paidPct}%` }} />
                    </div>
                    <span className="text-[10px] text-zinc-400 tabular-nums">{paidPct}% paid · {c.invoiceCount}</span>
                  </div>
                </div>
              </li>
            );
      ...
      <main className="p-6 space-y-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-0.5">Live snapshot of receivables, expenses, and inventory.</p>
          </div>
          <Link
            href="/admin/reports"
...
export default function AdminHomePage() {
  return <AdminAuthGate>{({ onLogout }) => <DashboardOverview onLogout={onLogout} />}</AdminAuthGate>;
}
