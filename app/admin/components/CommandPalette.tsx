...
export default function CommandPalette() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch('/api/admin/invoices').then((r) => (r.ok ? r.json() : [])).catch(() => []),
      fetch('/api/admin/quotes').then((r) => (r.ok ? r.json() : [])).catch(() => []),
      fetch('/api/admin/receipts').then((r) => (r.ok ? r.json() : [])).catch(() => []),
  ...
  const allHits = useMemo<Hit[]>(() => {
    ...
    for (const i of data.invoices) {
      out.push({ kind: 'invoice', title: `${i.number} · ${i.customer?.name || 'Unknown'}`, subtitle: `${i.status} · ${i.currency} ${i.total}`, href: '/admin/invoices' });
    }
    for (const q of data.quotes) {
      out.push({ kind: 'quote', title: `${q.number} · ${q.customer?.name || 'Unknown'}`, subtitle: `${q.status} · ${q.currency} ${q.total}`, href: '/admin/quotes' });
    }
    for (const r of data.receipts) {
      out.push({ kind: 'receipt', title: `${r.number} · ${r.customer?.name || 'Unknown'}`, subtitle: `${r.paymentMethod} · ${r.currency} ${r.total}`, href: '/admin/receipts' });
    }
    for (const c of data.clients) {
      out.push({ kind: 'client', title: c.name, subtitle: [c.company, c.email].filter(Boolean).join(' · '), href: '/admin/clients', keywords: `${c.email} ${c.phone}` });
    }
    for (const p of data.products) {
      out.push({ kind: 'product', title: p.name, subtitle: `${p.category} · ${p.currency} ${p.price}`, href: '/admin/products', keywords: `${p.slug} ${(p.tags || []).join(' ')}` });
    }
    return out;
  }, [data]);
  ...
  useEffect(() => {
    if (!query.trim()) {
      setHits(NAV_HITS.slice(0, 8));
      setSelected(0);
      return;
    }
    const q = query.trim();
    const scored = allHits
      .map((h) => ({ hit: h, score: Math.max(fuzzyScore(h.title, q), fuzzyScore(h.subtitle || '', q) - 50, fuzzyScore(h.keywords || '', q) - 50) }))
      .filter((x) => x.score > 0)
      .sort((a, b) => b.score - a.score)
      .slice(0, 20);
    setHits(scored.map((s) => s.hit));
    setSelected(0);
  }, [query, allHits]);
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 border-b border-zinc-100 px-4">
          <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8} className="text-zinc-400">
          ...
          />
          <kbd className="text-[10px] font-semibold text-zinc-400 border border-zinc-200 rounded px-1.5 py-0.5">ESC</kbd>
        </div>
          ...
          {hits.map((h, i) => (
            <button
              ...
              className={`flex w-full items-center gap-3 px-4 py-2.5 text-left transition ${
                i === selected ? 'bg-zinc-50' : 'hover:bg-zinc-50'
              }`}
            >
              <span className={`shrink-0 text-[10px] font-bold uppercase px-1.5 py-0.5 rounded ${KIND_TONE[h.kind]}`}>
                {KIND_LABEL[h.kind]}
              </span>
              ...
              {i === selected && (
                <kbd className="text-[10px] font-semibold text-zinc-400 border border-zinc-200 rounded px-1.5 py-0.5">↵</kbd>
              )}
            </button>
          ))}
        </div>
        <div className="flex items-center justify-between border-t border-zinc-100 px-4 py-2 text-[11px] text-zinc-400">
          <span>Press <kbd className="border border-zinc-200 rounded px-1">↑</kbd> <kbd className="border border-zinc-200 rounded px-1">↓</kbd> to navigate</span>
          <span><kbd className="border border-zinc-200 rounded px-1">⌘K</kbd> to open</span>
        </div>
      </div>
    </div>
  );
