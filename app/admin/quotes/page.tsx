...
export default function QuotesAdmin() {
  ...
  const saveQuote = async (quote: Quote) => {
    const normalized = recalc(quote);
    if (!normalized.customer.name.trim()) {
      setFormError('Add a customer name before saving this quote.');
      return;
    }
    if (
      normalized.lineItems.some(
        (item) => !item.description.trim() || item.quantity < 1 || item.unitPrice < 0,
      )
    ) {
      setFormError('Complete each line item with a description, quantity, and valid price.');
      return;
    }
    if (normalized.validUntil < normalized.issueDate) {
      setFormError('Valid until must be on or after the issue date.');
      return;
    }
    setFormError('');
    setSaving(true);
    try {
      ...
      if (res.ok) {
        ...
        setFormError(data.error || 'We could not save this quote. Check the fields and try again.');
      }
    } catch {
      setFormError('We could not save this quote. Check your connection and try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const sendEmailPdf = async () => {
    if (!printing || !printing.customer.email) {
    ...
    try {
      ...
      const element = document.getElementById('quote-print');
      if (!element) throw new Error('Print element not found');
      const canvas = await toCanvas(element, { quality: 0.95, pixelRatio: 2 });
      ...
      const res = await fetch('/api/admin/send-document', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          to: printing.customer.email,
          documentType: 'Quote',
          documentNumber: printing.number,
          customerName: printing.customer.name,
          total: printing.total.toFixed(2),
      ...
      const data = await res.json();
      setMessage(res.ok ? 'Email sent successfully' : data.error || 'Failed to send email');
    } catch (err) {
      ...
      <main className="p-6">
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <div className="rounded-xl border border-zinc-200 bg-white p-4">
            ...
            <p className="text-2xl font-bold text-green-600">
              {fmtCurrency(acceptedAmount, 'USD')}
            </p>
                ...
                {quotes.map((q) => (
                  <tr key={q.id} className="border-b border-zinc-50 hover:bg-zinc-50">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[q.status]}`}
                      >
                        {q.status}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1.5">
                        ...
                        <button
                          onClick={() => downloadRowPdf(q)}
                          disabled={downloading}
                          className="rounded-lg border border-zinc-900 bg-zinc-900 px-2.5 py-1 text-xs font-medium text-white hover:bg-zinc-700 disabled:opacity-60"
                        >
                          PDF
                        </button>
                        <button
        ...
        {editing && (
          ...
          >
            ...
            <div className="max-h-[100dvh] w-full max-w-3xl bg-white flex flex-col shadow-2xl">
              <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4">
                <h2
                  id="quote-editor-title"
                  className="font-heading text-sm font-bold text-zinc-900"
                >
                ...
                <button
                  ...
                  className="inline-flex h-10 w-10 items-center justify-center rounded-lg text-lg text-zinc-400 hover:bg-zinc-50 hover:text-zinc-600"
                >
                  &#10005;
                </button>
              </div>
              ...
              {formError && (
                <p
                  role="alert"
                  className="mx-5 mt-4 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm font-medium text-red-700"
                >
                  {formError}
                </p>
              )}
              ...
              >
                ...
                <fieldset className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditing({
                            ...editing,
                            customer: { ...editing.customer, email: e.target.value },
                          })
                        }
                        className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditing({
                            ...editing,
                            customer: { ...editing.customer, phone: e.target.value },
                          })
                        }
                        className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm"
                      />
                    ...
                    <div>
                      ...
                      <input
                        value={editing.customer.company || ''}
                        onChange={(e) =>
                          setEditing({
                            ...editing,
                            customer: { ...editing.customer, company: e.target.value },
                          })
                        }
                        className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm"
                      />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditing({
                          ...editing,
                          customer: { ...editing.customer, address: e.target.value },
                        })
                      }
                      className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm"
                    />
                ...
                {/* Line Items */}
                <fieldset className="space-y-3">
                  <legend className="text-xs font-bold text-zinc-500 uppercase tracking-wide">
                    Line Items
                  </legend>
                  <div className="space-y-2">
                    {editing.lineItems.map((li, idx) => (
                      <div key={li.id} className="space-y-1">
                        <div className="grid grid-cols-[minmax(0,1fr)_4.5rem_6.5rem_auto] items-end gap-2 sm:flex">
                          <div className="flex-1">
                            ...
                            <div className="flex gap-1">
                              <button
                                type="button"
                                onClick={() => {
                                  setShowProductPicker(showProductPicker === idx ? null : idx);
                                  setProductSearch({});
                                }}
                          ...
                          <div className="w-auto sm:w-28">
                            <label className="mb-1 block text-xs text-zinc-400">Unit Price</label>
                            <input
                              type="number"
                              min="0"
                              step="0.01"
                          ...
                          <div className="w-auto text-right sm:w-24">
                            <label className="mb-1 block text-xs text-zinc-400">Total</label>
                            <p className="py-2 text-sm font-medium text-zinc-700">
                              {fmtCurrency(li.quantity * li.unitPrice, editing.currency)}
                            </p>
                          </div>
                          <button
                            type="button"
                            ...
                            className="mb-1 inline-flex h-10 w-10 items-center justify-center rounded-lg text-zinc-400 hover:bg-red-50 hover:text-red-500"
                            title="Remove"
                          >
                            <svg
                              width="16"
                              ...
                              {products
                                .filter(
                                  (p) =>
                                    ...
                                    p.name
                                      .toLowerCase()
                                      .includes((productSearch[idx] || '').toLowerCase()),
                                )
                                .slice(0, 8)
                                .map((p) => (
                                  <button
                                  ...
                                  >
                                    ...
                                    <span className="ml-3 shrink-0 text-xs text-zinc-500 group-hover:text-red-100">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setEditing({ ...editing, status: e.target.value as QuoteStatus })
                          }
                          className="w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm"
                        >
                          {STATUSES.map((s) => (
                  ...
                  <div className="space-y-2 rounded-xl bg-zinc-50 p-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">
                        {fmtCurrency(recalc(editing).subtotal, editing.currency)}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditing(
                            recalc({ ...editing, taxRate: parseFloat(e.target.value) || 0 }),
                          )
                        }
                        className="w-20 ml-auto rounded-lg border border-zinc-200 px-2 py-1 text-sm text-right"
                      />
                    </div>
                    <div className="flex justify-between text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditing(
                            recalc({ ...editing, discount: parseFloat(e.target.value) || 0 }),
                          )
                        }
                        className="w-24 ml-auto rounded-lg border border-zinc-200 px-2 py-1 text-sm text-right"
                      />
                    </div>
            ...
            <div className="bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl rounded-xl print:shadow-none print:rounded-none print:max-h-none print:max-w-none">
              <div className="flex justify-between items-center p-4 border-b border-zinc-100 print:hidden">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => downloadPdf('quote-print', `${printing.number}.pdf`)}
                    disabled={downloading}
                  ...
                  {printing.customer.email && printing.customer.email.includes('@') ? (
                    <button
              ...
              <div className="p-10 print:p-0" id="quote-print">
                ...
                <div className="flex justify-between items-start mb-10">
                  <div>
                    ...
                    <p className="text-[11px] text-zinc-400">
                      {company?.tagline || 'Technology Solutions'}
                    </p>
                  </div>
                      ...
                      className={`mt-2 inline-block rounded px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${
                        printing.status === 'accepted'
                          ...
                          : printing.status === 'rejected'
                            ...
                            : printing.status === 'sent'
                              ? 'bg-blue-100 text-blue-700'
                              : printing.status === 'expired'
                ...
                <div className="grid grid-cols-3 gap-6 mb-8 pb-8 border-b border-zinc-200">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-zinc-900">
                      {company?.name || 'Cansan Solutions'}
                    </p>
                    {company?.addressLine1 && (
                      <p className="text-xs text-zinc-500 mt-0.5">{company.addressLine1}</p>
                    )}
                    {company?.addressLine2 && (
                      <p className="text-xs text-zinc-500">{company.addressLine2}</p>
                    )}
                    ...
                    {company?.phone && (
                      <p className="text-xs text-zinc-500 mt-1">{company.phone}</p>
                    )}
                    {company?.email && <p className="text-xs text-zinc-500">{company.email}</p>}
                    {company?.tinNumber && (
                      <p className="text-xs text-zinc-400 mt-1">TIN: {company.tinNumber}</p>
                    )}
                    {company?.vatNumber && (
                      <p className="text-xs text-zinc-400">VAT: {company.vatNumber}</p>
                    )}
                    {company?.vendorNumber && (
                      <p className="text-xs text-zinc-400">Vendor: {company.vendorNumber}</p>
                    )}
                  </div>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 mb-2">
                    ...
                    {printing.customer.company && (
                      <p className="text-xs text-zinc-500 mt-0.5">{printing.customer.company}</p>
                    )}
                    {printing.customer.email && (
                      <p className="text-xs text-zinc-500">{printing.customer.email}</p>
                    )}
                    {printing.customer.phone && (
                      <p className="text-xs text-zinc-500">{printing.customer.phone}</p>
                    )}
                  ...
                  <div className="text-right">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-semibold text-zinc-900">
                        {fmtDate(printing.issueDate)}
                      </p>
                      <p className="text-xs text-zinc-500 mt-2">Valid Until</p>
                      <p className="text-sm font-semibold text-zinc-900">
                        {fmtDate(printing.validUntil)}
                      </p>
                ...
                <table className="w-full mb-6 text-sm">
                  <thead>
                    <tr className="bg-zinc-900 text-white">
                      <th className="text-left px-4 py-2.5 text-[10px] font-bold uppercase tracking-wider rounded-l">
                        Description
                      </th>
                      <th className="text-center px-4 py-2.5 text-[10px] font-bold uppercase tracking-wider w-16">
                        Qty
                      </th>
                      <th className="text-right px-4 py-2.5 text-[10px] font-bold uppercase tracking-wider w-28">
                        Unit Price
                      </th>
                      <th className="text-right px-4 py-2.5 text-[10px] font-bold uppercase tracking-wider w-28 rounded-r">
                        Amount
                      </th>
                    </tr>
                  </thead>
                    ...
                    {printing.lineItems.map((li, i) => (
                      <tr key={li.id} className={i % 2 === 0 ? 'bg-white' : 'bg-zinc-50'}>
                        ...
                        <td className="px-4 py-3 text-right text-zinc-600">
                          {fmtCurrency(li.unitPrice, printing.currency)}
                        </td>
                        <td className="px-4 py-3 text-right font-semibold text-zinc-900">
                          {fmtCurrency(li.total, printing.currency)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <div className="w-72">
                    <div className="flex justify-between py-1.5 text-sm border-t border-zinc-100">
                      ...
                      <span className="text-zinc-800">
                        {fmtCurrency(printing.subtotal, printing.currency)}
                      </span>
                    </div>
                    {printing.taxRate > 0 && (
                      <div className="flex justify-between py-1.5 text-sm border-t border-zinc-100">
                        ...
                        <span className="text-zinc-800">
                          {fmtCurrency(printing.taxAmount, printing.currency)}
                        </span>
                      </div>
                    )}
                    {printing.discount > 0 && (
                      <div className="flex justify-between py-1.5 text-sm border-t border-zinc-100">
                        ...
                        <span className="text-green-600">
                          -{fmtCurrency(printing.discount, printing.currency)}
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between items-center mt-1 bg-zinc-900 text-white px-4 py-3 rounded">
                      ...
                      <span className="text-xl font-black">
                        {fmtCurrency(printing.total, printing.currency)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border-t-2 border-zinc-900 pt-4 flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <p className="text-[11px] text-zinc-500">
                      {company?.email || 'info@cansansolutions.co.zw'}
                    </p>
                    <p className="text-[11px] text-zinc-500">
                      {company?.phone || '+263 77 375 4747'}
                    </p>
                    {company?.website && (
                      <p className="text-[11px] text-zinc-400">{company.website}</p>
                    )}
                  </div>
                </div>
              </div>
                      ...
                      {clients
                        ...
                        .map((c) => {
                          ...
                          return (
                            ...
                            >
                              <div
                                className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-white font-bold text-sm ${color}`}
                              >
                ...
                {clientCreating && (
                  <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => setClientCreating(false)}
                        className="flex-1 rounded-lg border border-zinc-200 py-2 text-sm font-medium text-zinc-600"
                      >
                        Back
                      </button>
                      <button
                      ...
                      >
                        {clientSaving ? 'Saving...' : 'Create & Select'}
                      </button>
                    </div>
                  </div>
                )}
