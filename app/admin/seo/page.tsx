...
export default function SeoAdminPage() {
  ...
  const load = async (windowDays: number) => {
    ...
    try {
      ...
      const message =
        fetchError instanceof Error ? fetchError.message : 'Failed to load SEO analytics';
      setError(message);
    } finally {
      setLoading(false);
    }
        ...
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-zinc-500">
              Track search traffic, source quality, and top-performing pages
            </p>
          </div>
          ...
          <>
            ...
            <section className="mt-6 rounded-2xl border border-zinc-200 bg-white p-5">
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  <span
                    className={`rounded-full px-2.5 py-1 font-semibold ${
                      summary?.connectors?.ga4Configured
                  ...
                  <span
                    className={`rounded-full px-2.5 py-1 font-semibold ${
                      summary?.connectors?.gscConfigured
                        ? 'bg-emerald-100 text-emerald-700'
                        : 'bg-zinc-100 text-zinc-600'
                    }`}
                  >
                    Search Console{' '}
                    {summary?.connectors?.gscConfigured ? 'Connected' : 'Not configured'}
                  </span>
                </div>
              </div>
              ...
              {!summary?.connectors?.ga4Configured || !summary?.connectors?.gscConfigured ? (
                <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                  Configure env vars: `GOOGLE_SERVICE_ACCOUNT_JSON` (or
                  `GOOGLE_SERVICE_ACCOUNT_EMAIL` + `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY`),
                  `GA4_PROPERTY_ID`, and `GSC_SITE_URL`.
                </div>
              ) : null}
              ...
              <section className="mt-6 rounded-2xl border border-zinc-200 bg-white p-5">
                ...
                <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                  ...
                  <div className="rounded-lg border border-zinc-100 p-3">
                    ...
                    <p className="mt-1 text-xl font-bold text-zinc-900">
                      {formatPercent(summary.gsc.ctr * 100)}
                    </p>
                  </div>
                  <div className="rounded-lg border border-zinc-100 p-3">
                    <p className="text-xs text-zinc-400">Avg Position</p>
                    <p className="mt-1 text-xl font-bold text-zinc-900">
                      {summary.gsc.averagePosition.toFixed(1)}
                    </p>
                  </div>
                </div>
              ...
              <section className="rounded-2xl border border-zinc-200 bg-white p-5">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <p className="text-xs text-zinc-500">
                    Last tracked: {formatDate(summary?.lastTrackedAt)}
                  </p>
                </div>
                      ...
                      <div key={point.date} className="flex items-center gap-3">
                        ...
                        <div className="h-5 flex-1 overflow-hidden rounded-full bg-zinc-100">
                          <div
                            ...
                            style={{
                              width: `${(point.views / maxViews(summary.dailyViews)) * 100}%`,
                            }}
                          />
                        </div>
                      ...
                      <div key={row.label} className="flex items-center gap-3">
                        ...
                        <div className="h-5 flex-1 overflow-hidden rounded-full bg-zinc-100">
                          <div
                            ...
                            style={{
                              width: `${(row.views / Math.max(summary.topSources[0]?.views || 1, 1)) * 100}%`,
                            }}
                          />
                        </div>
                        <div className="w-10 text-right text-xs font-semibold text-zinc-700">
            ...
            <section className="mt-6 rounded-2xl border border-zinc-200 bg-white p-5">
              ...
              {summary?.topSearchTerms?.length ? (
                ...
                <p className="text-sm text-zinc-500">
                  No keyword terms detected yet. Terms will appear from organic referrers and UTM
                  term tracking.
                </p>
              )}
            </section>
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-2">
              <section className="rounded-2xl border border-zinc-200 bg-white p-5">
                ...
                {summary?.topClicks?.length ? (
                    ...
                    {summary.topClicks.map((row) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p
                            className="min-w-0 flex-1 truncate text-sm text-zinc-700"
                            title={target}
                          >
                            {target || '-'}
                          </p>
                  ...
                  <p className="text-sm text-zinc-500">
                    No CTA clicks recorded yet (WhatsApp, tel:, mailto:, outbound links, internal
                    CTAs).
                  </p>
                )}
              </section>
                ...
                {summary?.brokenUrls?.length ? (
                  <div className="space-y-2">
                    {summary.brokenUrls.map((row) => (
                      <div
                        key={row.label}
                        className="flex items-center gap-3 rounded-lg border border-red-100 bg-red-50/40 px-3 py-2"
                      >
                        <p
                          className="min-w-0 flex-1 truncate text-sm text-red-700"
                          title={row.label}
                        >
                          {row.label}
                        </p>
