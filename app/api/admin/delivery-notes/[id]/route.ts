...
import { getDeliveryNote, saveDeliveryNote, deleteDeliveryNote } from '@/lib/db';
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const saved = await saveDeliveryNote(merged);
    return NextResponse.json(saved);
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to update delivery note';
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const deleted = await deleteDeliveryNote(id);
