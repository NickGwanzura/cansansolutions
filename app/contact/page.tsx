...
const contactItems = [
  ...
  },
  {
    ...
    detail:
      'Corner Speke & 1st Street, Harare. Walk-ins welcome, call ahead to confirm availability.',
...
export default function ContactPage() {
  return (
    <div>
      ...
      <section className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
        <div className="grid gap-5 sm:grid-cols-2">
          {contactItems.map((item) => (
            <div
              key={item.label}
              className="group flex flex-col gap-5 rounded-3xl border border-zinc-200 bg-white p-6 shadow-[0_8px_26px_rgba(24,24,27,0.05)] transition hover:-translate-y-1 hover:border-red-200 hover:shadow-[0_16px_36px_rgba(220,38,38,0.1)] sm:p-7"
            >
              ...
              <a
                ...
                className={`mt-auto flex min-h-11 items-center justify-center gap-2 rounded-full px-4 py-2.5 text-sm font-semibold transition group-hover:shadow-md ${item.btnClass}`}
              >
                {item.btnLabel} <span aria-hidden="true">→</span>
              </a>
            </div>
          ))}
        </div>
      ...
      <section className="bg-zinc-50 px-4 py-14 sm:px-6 sm:py-16">
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-2">
          ...
          <div>
            ...
            <div className="divide-y divide-zinc-200 overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm">
              {hours.map((h) => (
                <div key={h.day} className="flex items-center justify-between px-5 py-4">
                  ...
                  <span
                    className={`text-sm font-semibold ${h.time === 'Closed' ? 'text-zinc-400' : 'text-zinc-900'}`}
                  >
                    {h.time}
                  </span>
                </div>
            ...
            <p className="mt-4 text-xs text-zinc-400">
              Outside hours? Send us a WhatsApp. We monitor messages and will reply as soon as
              possible.
            </p>
          </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              {faqs.map((faq) => (
                <details
                  key={faq.q}
                  className="group rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm transition open:border-red-200"
                >
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-4 text-sm font-semibold text-zinc-900">
                    {faq.q}
                    <span className="text-lg font-normal text-red-600 transition group-open:rotate-45">
                      +
                    </span>
                  </summary>
        ...
        <div className="mx-auto mt-10 max-w-6xl rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm sm:p-7">
          <p className="text-xs font-bold uppercase tracking-widest text-zinc-400">
            Helpful Before You Message
          </p>
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <a
                key={item.href}
                href={item.href}
                className="flex min-h-11 items-center justify-between rounded-2xl border border-zinc-200 px-4 py-3 text-sm font-semibold text-zinc-700 transition hover:border-red-300 hover:bg-red-50 hover:text-red-600"
              >
      ...
      {/* Final WhatsApp CTA */}
      <section className="bg-[radial-gradient(circle_at_50%_0%,#ef4444,#b91c1c)] px-6 py-14 text-center text-white">
        <h2 className="text-xl font-bold">Still have questions?</h2>
        <p className="mt-1 text-green-100 text-sm">
          Send us a message. We usually respond within minutes.
        </p>
        <a
