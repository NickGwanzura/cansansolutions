...
export function CartDrawer({ open, onClose }: Props) {
  ...
  // If cart becomes empty while on review step, go back to cart
  useEffect(() => {
    if (items.length === 0 && step === 'review') setStep('cart');
  }, [items.length, step]);
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4">
              <div className="flex flex-col gap-1.5">
                ...
                {items.length > 0 && <StepDots step={step} />}
              </div>
              <button
                onClick={onClose}
                          ...
                          {items.map((item) => (
                            ...
                            >
                              <Link
                                ...
                                onClick={onClose}
                                className="h-14 w-14 shrink-0 overflow-hidden rounded-lg bg-white border border-zinc-100 block"
                              >
                              ...
                              <div className="flex flex-1 flex-col gap-1 min-w-0">
                                <Link
                                  href={`/products/${item.slug}`}
                                  onClick={onClose}
                                  className="font-heading text-xs font-semibold text-zinc-900 leading-snug hover:text-red-600 transition-colors line-clamp-2"
                                >
                                  {item.name}
                                </Link>
                                ...
                                <div className="flex items-center gap-1.5 mt-0.5">
                                  <button
                                    ...
                                    className="flex h-6 w-6 items-center justify-center rounded-full border border-zinc-200 bg-white text-sm font-bold transition hover:border-red-300 hover:text-red-600 active:scale-90"
                                  >
                                    −
                                  </button>
                ...
                {step === 'review' && (
                  <>
                    <p className="text-[11px] text-zinc-400 leading-relaxed text-center">
                      Prices are indicative. Final price confirmed via WhatsApp before payment.
                    </p>
                    <motion.a
                      href={waHref}
                      target="_blank"
